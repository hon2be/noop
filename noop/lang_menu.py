...
def _draw(stdscr, cursor: int):
    ...
    try:
        stdscr.addstr(oy + 1, ox + max(0, (w - len(title)) // 2),
                      title[:w - 4], curses.A_BOLD)
    ...
    try:
        ...
        stdscr.addstr(footer_y, ox + max(0, (w - len(hint)) // 2),
                      hint[:w - 4], curses.A_DIM)
    except curses.error:
        pass
