...
def _draw_ad_box(stdscr, ad: Ad, oy: int, ox: int, w: int,
                 has_art: bool, box_h: int):
    """
    광고 박스 전체.
...
def _draw_choices(stdscr, oy: int, ox: int, w: int,
                  cursor: int, box_h: int):
    """
    [ ← 스킵 ]   [ 방문 → ]
    박스 하단 테두리 바로 아래 배치.
    """
...
def _draw_countdown(stdscr, oy: int, ox: int, w: int,
                    sec: float, box_h: int):
